...
- [`rfcs/`](../rfcs/) — public boundary contracts (wire formats,
  taxonomies, transports). Normative, versioned.
- [`decisions/`](../decisions/) — the 16 original MADR-lite ADRs
  (DR-0001 — DR-0016) anchoring the architecture.
- [`notebook/`](../notebook/) — BSPs, FSPs, and per-slice PLAN-S* files
  for the V1 notebook substrate (UI/host side).
- [`dev-guide/`](../dev-guide/) — narrative chapters from the original
  design conversation.
- [`bsps/`](../bsps/) — the older single-file BSP namespace.
- [`ops/`](../ops/) — operational runbooks.
...
- [RFC-001](../rfcs/RFC-001-mcp-tool-taxonomy.md) — MCP tool taxonomy
  (the agent-facing tool surface).
- [RFC-002](../rfcs/RFC-002-claude-code-provisioning.md) — Claude
  Code provisioning (system prompt template, allowed-tools, spawn
  argv).
- [RFC-003](../rfcs/RFC-003-custom-message-format.md) — custom
  message format (superseded in places by RFC-006 v2).
- [RFC-004](../rfcs/RFC-004-failure-modes.md) — failure-mode
  taxonomy + K-class error codes.
- [RFC-006 v2](../atoms/protocols/family-d-event-log.md) — envelope
  routing (Families A/B/C/D/F/G).
- [RFC-007](../rfcs/RFC-007-tape-otlp-logs.md) — tape capture +
  OTLP log records.
- [RFC-008](../rfcs/RFC-008-kernel-host-integration.md) — kernel
  ↔ host integration (PTY + socket).
- [RFC-009](../rfcs/RFC-009-zone-control-and-config.md) — zones
  and configuration.

**ADRs the kernel embodies:**
- [DR-0012](../decisions/0012-llmkernel-sole-kernel.md) — LLMKernel
  is the sole kernel (no Jupyter kernel).
- [DR-0015](../decisions/0015-kernel-extension-bidirectional-mcp.md)
  — kernel ↔ extension bidirectional MCP.
- [DR-0010](../decisions/0010-force-tool-use-suppress-text.md) —
  force tool use; suppress agent free-text.
- [DR-0016](../decisions/0016-rfc-standards-discipline.md) — RFC
  standards discipline.

**Substrate sequencing plans:**
- [BSP-004](../notebook/BSP-004-kernel-runtime.md) — kernel runtime
  (asyncio under uvicorn / FastAPI lifespan).
- [BSP-006](../notebook/BSP-006-embedded-asgi.md) — embedded ASGI.
...
4. Skim [RFC-008](../rfcs/RFC-008-kernel-host-integration.md) to
