...
**Goal**: ship the visual three-pane mental model (streaming / current / artifacts) via cell-status decorations + CSS, and fold in [FSP-002](../03%20-%20Blueprint/FSP-002-cell-search-collapse.md) (in-cell search + collapse all / expand all). Pure UX polish capping V1.
...
- The operator needs to see at a glance which cell is *streaming* (in-flight output), which is *current* (the one being edited), and which carries *artifacts* (overlay outputs / attachments) — per [BSP-005 §"S10"](../03%20-%20Blueprint/BSP-005-cell-roadmap.md).
...
Driver: [BSP-005 §"S10"](../03%20-%20Blueprint/BSP-005-cell-roadmap.md), folding [FSP-002](../03%20-%20Blueprint/FSP-002-cell-search-collapse.md). Atoms: [concepts/cell-kinds](../04%20-%20Reference/atoms/concepts/cell-kinds.md), [concepts/section](../04%20-%20Reference/atoms/concepts/section.md), [concepts/output-kind](../04%20-%20Reference/atoms/concepts/output-kind.md).
...
- Per-FSP-002 §4: V1 ships option-A persistence (extension-side workspace state) — collapse state lives in VS Code's `WorkspaceState`, not in `metadata.rts`. V2 promotes to `metadata.rts` per [decisions/v1-flat-sections](../04%20-%20Reference/atoms/decisions/v1-flat-sections.md) discipline.
...
1. **State enum.** Add `extension/src/types/cell-display-state.ts`:
   ...
   ```
   `streaming` is derived from the cell's RunFrame having `status: "running"`. `current` is `vscode.window.activeNotebookEditor.selection`. `artifact` is set when the cell carries an `output_kind` of `vnd.rts.artifact+json` per [output-kind atom](../04%20-%20Reference/atoms/concepts/output-kind.md).
...
- [concepts/cell-kinds.md](../04%20-%20Reference/atoms/concepts/cell-kinds.md) — referenced for the artifact-kind detection; no change.
- [concepts/section.md](../04%20-%20Reference/atoms/concepts/section.md) — sections affect "all cells" search scope; no atom change.
- [concepts/output-kind.md](../04%20-%20Reference/atoms/concepts/output-kind.md) — `vnd.rts.artifact+json` detection; no atom change.
