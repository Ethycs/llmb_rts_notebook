...
Driver: [BSP-002 §4.6](../03%20-%20Blueprint/BSP-002-conversation-graph.md). Slice spec: [BSP-005 §"S4"](../03%20-%20Blueprint/BSP-005-cell-roadmap.md). Atom: [continue-turn](../04%20-%20Reference/atoms/operations/continue-turn.md) §"Invariants / Preconditions" — the handoff rule already specifies the desired semantics.
...
- Handoff messages are NOT separate turns in the DAG (they are transient context injection per [continue-turn](../04%20-%20Reference/atoms/operations/continue-turn.md)).
...
- Partial-handoff strategies (drop oldest, summarize) when `missed_turn_count` exceeds budget. V2+ per [continue-turn](../04%20-%20Reference/atoms/operations/continue-turn.md) §"V1 vs V2+".
- Cell rendering of `[handoff]` markers — handoff messages stay invisible per [BSP-002 §6 Issue 3](../03%20-%20Blueprint/BSP-002-conversation-graph.md).
...
2. **Persistence.** Mirror `last_seen_turn_id` into `metadata.rts.zone.agents.<id>.session.last_seen_turn_id` per [agent atom schema](../04%20-%20Reference/atoms/concepts/agent.md). The field already exists in the atom schema; this slice wires it via the existing `update_agent_session` intent kind in [protocols/submit-intent-envelope](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md).
...
6. **`send_user_turn` delta** (`send_user_turn` already shipped in S3 per [contracts/agent-supervisor](../04%20-%20Reference/atoms/contracts/agent-supervisor.md); this extends it):
   ```python
...
Wire envelope (already shipped in S3 per [protocols/operator-action](../04%20-%20Reference/atoms/protocols/operator-action.md)):
...
| Risk | Mitigation |
|---|---|
| Missed-turn count grows unbounded in long notebooks | V1 emits all missed turns per [continue-turn V1 vs V2+](../04%20-%20Reference/atoms/operations/continue-turn.md); V2 layers a budget. Walker terminates at `last_seen_turn_id`. |
| Synthesis wording drifts between runs | Pin the prefix template literal in code; test asserts exact string. |
| stdin write race with agent's own response stream | Reader is on stdout; stdin writes serialized through `_lock` per [contracts/agent-supervisor §"Locking / threading"](../04%20-%20Reference/atoms/contracts/agent-supervisor.md). |
...
- [concepts/agent.md](../04%20-%20Reference/atoms/concepts/agent.md) — `last_seen_turn_id` field is already in the schema; this slice actively maintains it.
- [contracts/agent-supervisor.md](../04%20-%20Reference/atoms/contracts/agent-supervisor.md) — `send_user_turn` already in "Public method signatures" (S3); note that S4 extends its internal semantics.
- [operations/continue-turn.md](../04%20-%20Reference/atoms/operations/continue-turn.md) — Status `V1 shipped`; cross-agent handoff invariant is now enforced (was aspirational before S4).
- [protocols/submit-intent-envelope.md](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md) — `update_agent_session` parameter shape verified consistent with the new field.
- [contracts/metadata-writer.md](../04%20-%20Reference/atoms/contracts/metadata-writer.md) — verify the `update_agent_session` handler accepts `last_seen_turn_id`.
...
- [x] [contracts/agent-supervisor.md](../04%20-%20Reference/atoms/contracts/agent-supervisor.md) updated to note S4 extends `send_user_turn`.
