...
- **M1 — Comment cells.** Operator-prose cells (`kind: "markdown"` per [concepts/cell-kinds](../04%20-%20Reference/atoms/concepts/cell-kinds.md)) get first-class authoring affordances. `kind` is already wired in S0.5; this adds the UX.
- **M2 — Annotations.** Within-turn text annotations (highlights, notes) anchored to a span range, persisted as `add_overlay` intents per [concepts/overlay-commit](../04%20-%20Reference/atoms/concepts/overlay-commit.md).
- **M3 — Promoted cells.** Operator promotes a span out of an agent turn into its own cell. The kernel side ([operations/promote-span](../04%20-%20Reference/atoms/operations/promote-span.md)) already exists; M3 wires the operator-side gesture.
...
- **M2**: Operator selects text inside a cell output, right-click → "Annotate"; annotation persists as an `add_overlay` intent per [protocols/submit-intent-envelope](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md). Annotations re-render on file open.
- **M3**: Operator right-clicks on an agent span, "Promote to cell" — submits `promote_span` operation per [operations/promote-span](../04%20-%20Reference/atoms/operations/promote-span.md); a new cell appears immediately after the source cell.
...
- AI-authored annotations (V2+; per [discipline/scratch-beats-config](../04%20-%20Reference/atoms/discipline/scratch-beats-config.md), V1 keeps annotations operator-authored).
- Annotation threads / replies (V2+).
- M3 cross-cell promotion (lifting a span from one cell into a different section) — V1 places the promoted cell immediately after the source cell per [operations/promote-span](../04%20-%20Reference/atoms/operations/promote-span.md).
...
1. **Cell-creation hook.** When VS Code emits `vscode.workspace.onDidChangeNotebookDocument` with a new markdown cell, the extension's controller submits `set_cell_metadata` with `kind: "markdown"`, `bound_agent_id: null`. See [protocols/submit-intent-envelope](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md).
...
5. **Annotation model.** Annotations are `add_overlay` operations per [protocols/submit-intent-envelope](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md) with `parameters: { kind: "annotation", target_span_id, range: { start, end }, body, author: "operator" }`. See [concepts/overlay-commit](../04%20-%20Reference/atoms/concepts/overlay-commit.md) for the operation kinds list.
...
9. **Promotion command.** `extension/src/notebook/promote/promote-command.ts` registers `llmnb.promote.span`. Right-click on a span (text or tool-call output) → submits `apply_overlay_commit` with one `promote_span` operation per [operations/promote-span](../04%20-%20Reference/atoms/operations/promote-span.md).
...
- M2: `add_overlay` (existing per [protocols/submit-intent-envelope](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md) registry).
- M3: `apply_overlay_commit` containing `promote_span` (existing per [operations/promote-span](../04%20-%20Reference/atoms/operations/promote-span.md)).
...
| M2 annotation overlay grows unbounded with edits | Append-only with range supersession is correct per [discipline/save-is-git-style](../04%20-%20Reference/atoms/discipline/save-is-git-style.md); V2 may add compaction. |
...
| M3 produces an `artifact` kind which is V2-reserved per [concepts/cell-kinds](../04%20-%20Reference/atoms/concepts/cell-kinds.md) | The atom says reserved kinds render inert; M3 produces them but the renderer handles them via [concepts/output-kind](../04%20-%20Reference/atoms/concepts/output-kind.md) — verify this works in practice or scope D7 to map to `agent` for V1. Decision flagged for operator. |
...
- [concepts/cell-kinds.md](../04%20-%20Reference/atoms/concepts/cell-kinds.md) — `markdown` row's "Shipped" status confirmed live in M1; `artifact` reserved-kind handling decision flagged in M3 risk.
- [operations/promote-span.md](../04%20-%20Reference/atoms/operations/promote-span.md) — Status flips to `V1 shipped (operator-side gesture wired)`.
- [protocols/submit-intent-envelope.md](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md) — `add_overlay` row's atom link points at a real annotation use case after M2.
- [concepts/overlay-commit.md](../04%20-%20Reference/atoms/concepts/overlay-commit.md) — `add_overlay` and `promote_span` entries among the 17 operation kinds now actively used.
- [discipline/save-is-git-style.md](../04%20-%20Reference/atoms/discipline/save-is-git-style.md) — annotations as commits reinforce this discipline; no change.
- [concepts/run-frame.md](../04%20-%20Reference/atoms/concepts/run-frame.md) — M4 panel reads RunFrames; no shape change.
