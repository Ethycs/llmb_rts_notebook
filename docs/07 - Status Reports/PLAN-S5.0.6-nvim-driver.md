...
1. **The kernel-side substrate is complete; only the UI is missing.** After S5.0.3 (driver extraction + `--connect`), S5.0.5 Phase 1 (multi-format `@@import`), and S5.0.5 Phase 2 (`@@export`), every notebook operation an operator could want is reachable via either:
   - a TCP envelope to a `llmnb serve` kernel ([transport-mode.md](../04%20-%20Reference/atoms/concepts/transport-mode.md)), or
   - in-kernel magic dispatch (`@@import` / `@@export` / generators).

   What's NOT available is an operator-facing surface that ISN'T the VS Code extension. The `llmnb execute --connect` CLI works for batch runs ([driver.md](../04%20-%20Reference/atoms/concepts/driver.md)) but has no per-cell semantics — every invocation re-runs the whole file. The nvim driver fills that gap.
...
- `--connect tcp://…` ([commit `df95ad4`](../03%20-%20Blueprint)) on `llmnb execute` is the precedent for how external clients attach. The sidecar reuses [`llm_client.boot.connect_to_kernel`](../../llm_client/boot.py).
...
| [`docs/atoms/concepts/driver.md`](../04%20-%20Reference/atoms/concepts/driver.md) | Update V1 driver inventory table — add "Nvim driver" row with status pin. |
| [`README.md`](../../README.md) | Add a "Drivers" section listing VS Code + nvim, with one-line setup for each. |
| **NEW** `docs/ops/validate-nvim-driver.md` | Operational guide — smoke-test recipe: start kernel, open `.magic` in nvim, run a cell, verify output appears. Mirrors [`docs/ops/validate-serve-mode.md`](../02%20-%20Implementation/ops/validate-serve-mode.md). |
...
| [`docs/atoms/concepts/driver.md`](../04%20-%20Reference/atoms/concepts/driver.md) | V1 driver inventory: add Nvim row | ~5 LoC |
...
4. **Operator smoke** — the procedure in [`docs/ops/validate-nvim-driver.md`](../02%20-%20Implementation/ops/validate-nvim-driver.md) (or equivalent) succeeds end-to-end on Linux. Run cell → output renders; run `@@export` cell → file lands; run `@@import` in another buffer → cells appear.
...
- [docs/atoms/concepts/driver.md](../04%20-%20Reference/atoms/concepts/driver.md) — driver concept atom; V1 inventory gains Nvim
- [docs/atoms/concepts/transport-mode.md](../04%20-%20Reference/atoms/concepts/transport-mode.md) — TCP transport the sidecar uses
- [docs/atoms/discipline/wire-as-public-api.md](../04%20-%20Reference/atoms/discipline/wire-as-public-api.md) — the contract a second driver validates
- [docs/atoms/protocols/wire-handshake.md](../04%20-%20Reference/atoms/protocols/wire-handshake.md) — first envelope the sidecar exchanges
...
- [docs/ops/validate-serve-mode.md](../02%20-%20Implementation/ops/validate-serve-mode.md) — operational guide whose pattern `validate-nvim-driver.md` mirrors
