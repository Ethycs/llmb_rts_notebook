...
**Atom anchors**: definitional content lives in [`docs/atoms/`](../04%20-%20Reference/atoms/README.md) — see `concepts/`, `operations/`, `decisions/`. This BSP stays normative for slice order, sizing, and dependency choreography.
...
The Issue 2 amendment (2026-04-28) inserts S0.5 ahead of S1, S3.5 between S3 and S4, and S5.5 between S5 and S6. S6 is expanded to also include RunFrame minimal per [decisions/v1-runframe-minimal](../04%20-%20Reference/atoms/decisions/v1-runframe-minimal.md). See §6 below for the amendment ratification.
...
**Concrete work**:
- Add `metadata.rts.cells[<id>].kind` field to the writer per the [cell-kinds atom](../04%20-%20Reference/atoms/concepts/cell-kinds.md).
...
**Atoms touched**: [cell-kinds](../04%20-%20Reference/atoms/concepts/cell-kinds.md), [cell](../04%20-%20Reference/atoms/concepts/cell.md), [decisions/v1-flat-sections](../04%20-%20Reference/atoms/decisions/v1-flat-sections.md).
...
**Atoms touched**: [cell](../04%20-%20Reference/atoms/concepts/cell.md), [agent](../04%20-%20Reference/atoms/concepts/agent.md), [cell-kinds](../04%20-%20Reference/atoms/concepts/cell-kinds.md).
...
**Atoms touched**: [agent](../04%20-%20Reference/atoms/concepts/agent.md), [decisions/no-rebind-popen](../04%20-%20Reference/atoms/decisions/no-rebind-popen.md).
...
**Atoms touched**: [operations/continue-turn](../04%20-%20Reference/atoms/operations/continue-turn.md), [turn](../04%20-%20Reference/atoms/concepts/turn.md), [agent](../04%20-%20Reference/atoms/concepts/agent.md).
...
**Concrete work**:
- Implement the walker per [decisions/v1-contextpacker-walk](../04%20-%20Reference/atoms/decisions/v1-contextpacker-walk.md): pinned anywhere → previous cells in current section in chronological order → current cell's prior sub-turns → exclude `scratch | excluded | obsolete` → dedupe preserving first-occurrence order.
- Persist outputs as [context-manifest](../04%20-%20Reference/atoms/concepts/context-manifest.md) entries via the new `record_context_manifest` intent (BSP-003 §5 amendment).
...
**Atoms touched**: [context-manifest](../04%20-%20Reference/atoms/concepts/context-manifest.md), [decisions/v1-contextpacker-walk](../04%20-%20Reference/atoms/decisions/v1-contextpacker-walk.md), [section](../04%20-%20Reference/atoms/concepts/section.md).
...
**Atoms touched**: [agent](../04%20-%20Reference/atoms/concepts/agent.md), [context-manifest](../04%20-%20Reference/atoms/concepts/context-manifest.md), [turn](../04%20-%20Reference/atoms/concepts/turn.md).
...
**Atoms touched**: [operations/branch-agent](../04%20-%20Reference/atoms/operations/branch-agent.md), [operations/revert-agent](../04%20-%20Reference/atoms/operations/revert-agent.md), [operations/stop-agent](../04%20-%20Reference/atoms/operations/stop-agent.md).
...
**Concrete work**:
- Implement `metadata.rts.zone.sections[]` per the [section atom](../04%20-%20Reference/atoms/concepts/section.md) and BSP-002 §13.1.1.
- Flat sections only in V1 per [decisions/v1-flat-sections](../04%20-%20Reference/atoms/decisions/v1-flat-sections.md): `parent_section_id` slot exists but MUST be `null`; non-null is rejected.
...
- Operations: [create-section](../04%20-%20Reference/atoms/operations/create-section.md), [rename-section](../04%20-%20Reference/atoms/operations/rename-section.md), [delete-section](../04%20-%20Reference/atoms/operations/delete-section.md). Deletion of a non-empty section is forbidden (decision SD1).
...
**Atoms touched**: [section](../04%20-%20Reference/atoms/concepts/section.md), [decisions/v1-flat-sections](../04%20-%20Reference/atoms/decisions/v1-flat-sections.md), [operations/create-section](../04%20-%20Reference/atoms/operations/create-section.md).
...
- **RunFrame minimal**: write `metadata.rts.zone.run_frames.<run_id>` per [decisions/v1-runframe-minimal](../04%20-%20Reference/atoms/decisions/v1-runframe-minimal.md) — `{run_id, cell_id, executor_id, turn_head_before, turn_head_after, context_manifest_id, status, started_at, ended_at}`. Deferred V2 fields (`parent_run_id`, `source_snapshot_id`, `overlay_commit_id`, `artifact_windows[]`, full `tool_permissions`) NOT emitted in V1; consumers tolerate absence.
- Inspect mode reads the RunFrame + manifest pair to render the per-run "what the agent saw" view.

**S6 expanded** to include RunFrame minimal per [decisions/v1-runframe-minimal](../04%20-%20Reference/atoms/decisions/v1-runframe-minimal.md). The two write-backs land together because both flow through the same `MetadataWriter` intent path; splitting them would double the test fixtures without reducing risk.
...
**Atoms touched**: [cell](../04%20-%20Reference/atoms/concepts/cell.md), [turn](../04%20-%20Reference/atoms/concepts/turn.md), [run-frame](../04%20-%20Reference/atoms/concepts/run-frame.md), [decisions/v1-runframe-minimal](../04%20-%20Reference/atoms/decisions/v1-runframe-minimal.md).
...
**Atoms touched**: [section](../04%20-%20Reference/atoms/concepts/section.md), [agent](../04%20-%20Reference/atoms/concepts/agent.md), [zone](../04%20-%20Reference/atoms/concepts/zone.md).
...
**Atoms touched**: [span](../04%20-%20Reference/atoms/concepts/span.md), [output-kind](../04%20-%20Reference/atoms/concepts/output-kind.md), [tool-call](../04%20-%20Reference/atoms/concepts/tool-call.md).
...
**Atoms touched**: [agent](../04%20-%20Reference/atoms/concepts/agent.md), [operations/stop-agent](../04%20-%20Reference/atoms/operations/stop-agent.md).
...
**Atoms touched**: [cell-kinds](../04%20-%20Reference/atoms/concepts/cell-kinds.md), [section](../04%20-%20Reference/atoms/concepts/section.md), [output-kind](../04%20-%20Reference/atoms/concepts/output-kind.md).
...
Adds `metadata.rts.cells[<id>].kind` field per the [cell-kinds atom](../04%20-%20Reference/atoms/concepts/cell-kinds.md). V1 ships four kinds (`agent | markdown | scratch | checkpoint`); reserves four V2+ kinds (`tool | artifact | control | native`) as forward-compat slots.

**Estimated**: 0.5 days. **Atoms touched**: [cell-kinds](../04%20-%20Reference/atoms/concepts/cell-kinds.md), [cell](../04%20-%20Reference/atoms/concepts/cell.md), [decisions/v1-flat-sections](../04%20-%20Reference/atoms/decisions/v1-flat-sections.md).
...
Implements the V1 ContextPacker as a pure, deterministic structural walker per [decisions/v1-contextpacker-walk](../04%20-%20Reference/atoms/decisions/v1-contextpacker-walk.md). Manifest persisted as [context-manifest](../04%20-%20Reference/atoms/concepts/context-manifest.md) entries via the new `record_context_manifest` BSP-003 intent.

**Estimated**: 1 day. **Atoms touched**: [context-manifest](../04%20-%20Reference/atoms/concepts/context-manifest.md), [decisions/v1-contextpacker-walk](../04%20-%20Reference/atoms/decisions/v1-contextpacker-walk.md), [section](../04%20-%20Reference/atoms/concepts/section.md).

### 6.3 S5.5 — Sections (overlay-graph narrative range) (NEW)

**Position**: between S5 and S6.
**Driver**: KB-target §0.1, §6.
**Status**: V1 shipped 2026-05-17 (kernel + commands + decoration + cell magic + extension recognition). See [PLAN-S5.5-sections.md](../07%20-%20Status%20Reports/PLAN-S5.5-sections.md) for the per-phase ship narrative.

Implements `metadata.rts.zone.sections[]` per the [section atom](../04%20-%20Reference/atoms/concepts/section.md). Flat sections only in V1 per [decisions/v1-flat-sections](../04%20-%20Reference/atoms/decisions/v1-flat-sections.md) — `parent_section_id` slot exists but MUST be `null`. Operations land via overlay commits ([create-section](../04%20-%20Reference/atoms/operations/create-section.md), [rename-section](../04%20-%20Reference/atoms/operations/rename-section.md), [delete-section](../04%20-%20Reference/atoms/operations/delete-section.md), [set-section-status](../04%20-%20Reference/atoms/operations/set-section-status.md)).

**Estimated**: 1.5 days. **Actual**: 1 wall-clock day (multi-phase, single agent). **Atoms touched**: [section](../04%20-%20Reference/atoms/concepts/section.md), [decisions/v1-flat-sections](../04%20-%20Reference/atoms/decisions/v1-flat-sections.md), [decisions/v1-section-status-interruptibility](../04%20-%20Reference/atoms/decisions/v1-section-status-interruptibility.md), [operations/create-section](../04%20-%20Reference/atoms/operations/create-section.md), [operations/rename-section](../04%20-%20Reference/atoms/operations/rename-section.md), [operations/delete-section](../04%20-%20Reference/atoms/operations/delete-section.md), [operations/set-section-status](../04%20-%20Reference/atoms/operations/set-section-status.md).
...
S6 now also writes `metadata.rts.zone.run_frames.<run_id>` per [decisions/v1-runframe-minimal](../04%20-%20Reference/atoms/decisions/v1-runframe-minimal.md). RunFrames pair with the S3.5 manifests so Inspect mode answers per-cell-run "what did the agent see and what changed in the turn DAG?"

**Estimated**: +1 day folded into S6 (S6 sized at 2 days total for cell-binding + RunFrame minimal). **Atoms touched**: [run-frame](../04%20-%20Reference/atoms/concepts/run-frame.md), [decisions/v1-runframe-minimal](../04%20-%20Reference/atoms/decisions/v1-runframe-minimal.md).
...
| Git-style + sections | S5 + S5.5 | ~3.5 d | ✅ S5a (`5b5533e`), S5b-revert (submodule `d85c3f4`), S5c-stop (submodule `4461794`); 🔵 S5.5 sections design locked in [PLAN-S5.5](../07%20-%20Status%20Reports/PLAN-S5.5-sections.md), implementation queued |
...
**Velocity reality**: the BSP-005 "working day" unit was sized for one mega-round agent in series. Observed wall-clock velocity with multi-agent parallel execution + atom-defined contracts is **~10× the budget**. V1 UX shipped during 2026-05-02 → 2026-05-19; the last two V1 ship-ready rows (row 13 substrate gaps, row 14 atom drift) closed 2026-06-30 — 908/908 kernel test suite green + all 3 atom-hygiene verification checks empty. **V1 is SHA-ready pending operator sign-off** (see [PLAN-v1-roadmap §5](../07%20-%20Status%20Reports/PLAN-v1-roadmap.md)). S5.0.6 (nvim driver) deferred 2026-05-19 — design locked at `8639949`, implementation picked up later when the nvim operator's dogfooding pressure justifies it; the `llmnb execute --connect` CLI (`df95ad4`) covers file-level operation in the interim. Per-cell gutter decoration deferred to whenever VS Code exposes `notebookCellDecoration` as a proposed API.

Don't pace to the "days" budget — pace to the slice ordering + dependency graph, which remain normative.

> **Magic vocabulary note**: §2 above references the legacy `/spawn` and `@<id>:` directive shapes. The current canonical syntax is the `@@<kind>` cell-magic + `@<flag>` line-magic vocabulary; see [PLAN-S5.0-cell-magic-vocabulary.md](../07%20-%20Status%20Reports/PLAN-S5.0-cell-magic-vocabulary.md) and [magic atom](../04%20-%20Reference/atoms/concepts/magic.md). The slice ladder itself is syntax-agnostic.
...
|---|---|---|
| **V2 branch-switching UX** — sidebar Branches subnode per agent with forked descendants; lineage recovered from `event_log[*]` `fork_agent` envelopes | extends §S7 agents tree; consumes data from §S5a `/branch` | ✅ shipped 2026-05-20 (`667dd68`) — [PLAN-V2-branch-switching-ux](../07%20-%20Status%20Reports/PLAN-V2-branch-switching-ux.md) |
| **V2 output-kind lens UI (sidebar grouping)** — 5th sidebar view grouping every span tagged with `llmnb.output.kind` by kind; high-attention kinds first; forward-compat `<other>` bucket | consumes the V1 OTLP tag stream per [output-kind atom](../04%20-%20Reference/atoms/concepts/output-kind.md) | ✅ shipped 2026-05-29 (`85bd5e4`) — [PLAN-V2-output-kind-lens](../07%20-%20Status%20Reports/PLAN-V2-output-kind-lens.md) |
...
| Multi-provider campaign (`gpt-cli` / `gemini` / `ollama`) — agents beyond `claude-code` | extends [agent atom](../04%20-%20Reference/atoms/concepts/agent.md) V1-vs-V2+ "V2+ providers" | 🔵 design doc not yet authored; multi-session campaign — `AgentSupervisor` Provider abstraction + per-provider drivers |
