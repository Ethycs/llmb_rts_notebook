...
**Status**: V1 ship 2026-05-19 — search UX shipped as a sidebar WebviewView (input + scope + case/word/regex toggles + M-of-N + click-to-reveal); bulk-collapse + find wrappers fire the engine builtins. The original "floating above the editor" pixel placement remains deferred to V2 pending VS Code overlay API; everything else from §2.1 is live.
**Targeting**: V2 (in-editor floating bar + auto-expand-on-match) per VERSIONING.md; V1 ships every operator-visible UX bullet from §2.1 in the sidebar
...
- **2026-05-19** (V1 partial-ship via PLAN-S10 reduced scope): the engine-native subset of this FSP shipped as wrapper commands — `llmnb.findInCells` fans out to the built-in `actions.find` (same widget Ctrl+F triggers when a notebook editor is focused), and `llmnb.collapseAllInputs` / `llmnb.collapseAllOutputs` / `llmnb.expandAllInputs` / `llmnb.expandAllOutputs` fan out to the engine's `notebook.cell.{collapse,expand}AllCell{Input,Output}` builtins.
- **2026-05-19** (V1 ship of §2.1 via sidebar WebviewView): the search UX now lives in a 4th view of the `llmnb` activity-bar container (`llmnb.search`, type `webview`). It delivers the full §2.1 surface — query input, scope filter (all / inputs / outputs / tool_calls / selected), case-sensitive / whole-word / regex toggles, M-of-N counter, prev/next + Enter / Shift+Enter / Esc keyboard, click-to-reveal via `llmnb.revealCell`, debounced live results (200ms), and F70 invalid-regex tooltip. The literal "floating above the editor" pixel placement (§2.1 first row) remains V2 because VS Code v1.92 has no overlay API and `vscode-jupyter` independently doesn't ship one either — the sidebar placement is the same UX in a different position. Auto-expand-on-collapsed-cell-match (§2.1 "Match highlighting" last sentence) remains V2 because per-cell collapse isn't extension-API-controllable in V1. The bulk-collapse WorkspaceState persistence (§4 option A) also remains V2 because `NotebookCell.metadata` is read-only at runtime and the engine handles its own per-cell chevron state.
