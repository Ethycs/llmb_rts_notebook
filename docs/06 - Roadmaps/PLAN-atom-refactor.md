...
**Status**: Executed 2026-04-28 (Phase 1 `f8223bf` → Phase 4 `4207504`); §4 row D1 superseded 2026-04-29 by [decisions/v1-section-status-interruptibility](../04%20-%20Reference/atoms/decisions/v1-section-status-interruptibility.md). Plan retained as the historical execution record + the canonical 24-row V1 decision table referenced by atom Status fields.
...
|---|---|---|
| D1 | Section status enum values | **SUPERSEDED 2026-04-29**: status enum ships in V1 as the section-level interruptibility lock (`open / in_progress / complete / frozen`). See [decisions/v1-section-status-interruptibility](../04%20-%20Reference/atoms/decisions/v1-section-status-interruptibility.md). Original guidance was to drop the enum; the reframe gives it real V1 consumers (every structural op gains a section-status precondition). |
...
1. `docs/notebook/KB-notebook-target.md` — body still has prose definitions of Section, Cell, Sub-turn, RunFrame, etc. Replace with atom links: `See [Cell](../03%20-%20Blueprint/atoms/concepts/cell.md), [Section](../03%20-%20Blueprint/atoms/concepts/section.md).` Keep the architectural narrative. **§0 V1 amendments collapse — they're now in `decisions/v1-*.md` atoms.**
...
3. **Link integrity**: every link resolves. Use markdown-link-check or a simple grep for `[*](../03%20-%20Blueprint/atoms/*)` patterns.
