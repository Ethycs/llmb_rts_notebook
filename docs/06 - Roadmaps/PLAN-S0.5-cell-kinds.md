...
- **Merge correctness ([S5.5 / S6](../03%20-%20Blueprint/BSP-005-cell-roadmap.md))** requires `c_a.kind == c_b.kind` as a precondition. Without an enum it is content-heuristic.
- **ContextPacker ([S3.5](PLAN-S3.5-context-packer.md))** must filter `scratch` and `checkpoint` cells differently from `agent`/`markdown`.
- **Render dispatch ([S1 — already shipped](../03%20-%20Blueprint/BSP-005-cell-roadmap.md#s1--cell-as-agent-identity-bsp-002-6--extension-only))** wants the kind label alongside the agent badge.
...
S0.5 lands the slot. Driver: [KB-notebook-target.md §0.4](../03%20-%20Blueprint/KB-notebook-target.md), ratified into the slice ladder via [BSP-005 §6.1](../03%20-%20Blueprint/BSP-005-cell-roadmap.md#61-s05--cell-kinds-typed-enum-new).
...
1. **Schema land — atom is the source of truth.** Read [docs/atoms/concepts/cell-kinds.md](../04%20-%20Reference/atoms/concepts/cell-kinds.md) verbatim. The eight enum values + per-kind constraints live there; do not re-define them in code comments — link.
...
2. **Writer field.** In `vendor/LLMKernel/llm_kernel/metadata_writer.py`:
   ...
   - Emit a Family F snapshot delta carrying the new field. See [protocols/family-f-notebook-metadata](../04%20-%20Reference/atoms/protocols/family-f-notebook-metadata.md).
...
There is one new wire change: `set_cell_metadata` accepts the `kind` field. The wider intent envelope is unchanged (see [protocols/submit-intent-envelope](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md)).
...
| Extension and kernel disagree on the enum | Single source of truth = the [cell-kinds atom](../04%20-%20Reference/atoms/concepts/cell-kinds.md). TypeScript and Python both reference the atom's eight values literally; PR review verifies. |
...
| Reserved kinds get accidentally dispatched | The reserved kinds have NO handler in S0.5; the renderer falls through to a "kind-label only" view per [cell-kinds atom invariants](../04%20-%20Reference/atoms/concepts/cell-kinds.md). |

## §7. Atoms touched + Atom Status fields needing update

- [concepts/cell-kinds.md](../04%20-%20Reference/atoms/concepts/cell-kinds.md) — already says "V1 shipped (4 kinds active, 4 reserved)" but the field is not yet implemented. After this slice, the status remains correct; verify the atom's Status line after merge.
- [concepts/cell.md](../04%20-%20Reference/atoms/concepts/cell.md) — Status `V1 shipped`; should call out S0.5 as the slice that lands the `kind` field. Update its `## V1 vs V2+` line if it claims V1 already has kind enforcement.
- [decisions/v1-flat-sections.md](../04%20-%20Reference/atoms/decisions/v1-flat-sections.md) — referenced by cell-kinds; no change.
- [contracts/metadata-writer.md](../04%20-%20Reference/atoms/contracts/metadata-writer.md) — Code drift section: add a line clearing the `set_cell_metadata.kind` drift after this slice lands.
- [protocols/submit-intent-envelope.md](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md) — `set_cell_metadata` row's "atom" link now reaches a real-implementation path.
...
- [ ] [docs/atoms/concepts/cell-kinds.md](../04%20-%20Reference/atoms/concepts/cell-kinds.md) Status line is verified consistent with implementation.
