...
[BSP-002 §3](../03%20-%20Blueprint/BSP-002-conversation-graph.md) defines three git-style operations on agent histories:
...
These were originally drafted as `/branch`, `/revert`, `/stop` slash directives (the PLAN-S5 first draft predates S5.0). After S5.0 shipped the `@`/`@@` magic vocabulary, the canonical operator surface for all three is **line magic** (`@`, not `@@`) — a parametric per-cell effect, NOT a cell-kind declaration. See [magic atom](../04%20-%20Reference/atoms/concepts/magic.md) line-magic registry: `revert / stop / branch | stub (S5)`. PLAN-S5.0 §3.4 lines 124-126 register the names with `pending_slice="S5"`; this slice flips them from stub to active.

[stop-agent](../04%20-%20Reference/atoms/operations/stop-agent.md) Status is `V1 shipped` (the supervisor already has stop-on-idle plumbing); [branch-agent](../04%20-%20Reference/atoms/operations/branch-agent.md) and [revert-agent](../04%20-%20Reference/atoms/operations/revert-agent.md) Status is `V1 spec'd (data model in BSP-002 §2; full UX V2+)`. Verifying against `vendor/LLMKernel/llm_kernel/agent_supervisor.py`: `fork`, explicit `revert`, and explicit `stop(agent_id)` methods are NOT present; the `agent_branch / agent_revert / agent_stop` action_types are catalogued in [protocols/operator-action](../04%20-%20Reference/atoms/protocols/operator-action.md) but no kernel handler dispatches to them. Writer intents `fork_agent` and `move_agent_head` are listed in `metadata_writer.py` but the handlers are stubs (per [PLAN-substrate-gap-closure](PLAN-substrate-gap-closure.md) gap G4).

Driver: [BSP-005 §"S5"](../03%20-%20Blueprint/BSP-005-cell-roadmap.md). Atoms: [branch-agent](../04%20-%20Reference/atoms/operations/branch-agent.md), [revert-agent](../04%20-%20Reference/atoms/operations/revert-agent.md), [stop-agent](../04%20-%20Reference/atoms/operations/stop-agent.md), [agent](../04%20-%20Reference/atoms/concepts/agent.md), [magic](../04%20-%20Reference/atoms/concepts/magic.md).
...
- Turn DAG materialization at `metadata.rts.zone.agents.<id>.turns[]` per [turn atom](../04%20-%20Reference/atoms/concepts/turn.md) Status `V1 shipped` — was an open item in the original PLAN-S5; now closed upstream.

## §2. Goals and non-goals

### Goals

- `AgentSupervisor.fork(source, at_turn_id, new_agent_id) → AgentHandle` per [contracts/agent-supervisor](../04%20-%20Reference/atoms/contracts/agent-supervisor.md).
...
- Both Case A (head) and Case B (ancestor) branch mechanics work per [branch-agent §"Two cases"](../04%20-%20Reference/atoms/operations/branch-agent.md).
...
- Branch-switching UX in the notebook view (sidebar/picker per [BSP-002 §11.2](../03%20-%20Blueprint/BSP-002-conversation-graph.md)) — V2+ per [branch-agent §V1 vs V2+](../04%20-%20Reference/atoms/operations/branch-agent.md).
...
**V2+ deferred:**
- **Reflog**: a per-agent history of HEAD movements that survives revert. V1 records `agent_ref_move` event-log entries (per [revert-agent](../04%20-%20Reference/atoms/operations/revert-agent.md)); a navigable reflog UI is V2+.
...
- **Soft revert** (keep session alive when target reachable via claude's resume) per [revert-agent §V2+](../04%20-%20Reference/atoms/operations/revert-agent.md).

Per [discipline/zachtronics](../04%20-%20Reference/atoms/discipline/zachtronics.md), every V1 control-flow effect is visible in the issuing cell's text — no hidden routing state.
...
2. **`AgentSupervisor.fork(source_agent, at_turn_id, new_agent_id) → AgentHandle`** per [contracts/agent-supervisor §"Spec'd but not yet present"](../04%20-%20Reference/atoms/contracts/agent-supervisor.md):
   ...
   - Validate `new_agent_id` is unique in the zone and not a reserved magic name; else K-class per [magic §K-class](../04%20-%20Reference/atoms/concepts/magic.md) (K32) or spawn-uniqueness error.
...
3. **`AgentSupervisor.revert(agent_id, to_turn_id) → None`** per [revert-agent](../04%20-%20Reference/atoms/operations/revert-agent.md):
   ...
   - Submit `record_event` with kind `agent_ref_move` and `reason: "operator_revert"` per [protocols/family-d-event-log](../04%20-%20Reference/atoms/protocols/family-d-event-log.md).
   - Fresh `claude_session_id` is assigned on the next continuation per [agent atom](../04%20-%20Reference/atoms/concepts/agent.md) invariants — pre-revert turns retain their original session id.

4. **`AgentSupervisor.stop(agent_id) → None`** per [stop-agent](../04%20-%20Reference/atoms/operations/stop-agent.md):
   - Validate agent exists; else **K20**.
...
8. **Event log entries.** Every `@branch`, `@revert`, `@stop` writes a `record_event` intent so Inspect mode can replay "alpha was reverted to t_2 at 14:03." See [protocols/family-d-event-log](../04%20-%20Reference/atoms/protocols/family-d-event-log.md).
...
Wire envelopes (already catalogued in [protocols/operator-action](../04%20-%20Reference/atoms/protocols/operator-action.md) lines 47-49):
...
| Case B replay diverges from Case A by claude version drift | Both invocations pin to the same claude binary version per [contracts/agent-supervisor §"Pre-spawn validation"](../04%20-%20Reference/atoms/contracts/agent-supervisor.md). |
...
- [contracts/agent-supervisor.md](../04%20-%20Reference/atoms/contracts/agent-supervisor.md) — `fork`, `revert`, `stop` move from "Spec'd but not yet present" to public methods; update Status / "Code drift vs spec".
- [operations/branch-agent.md](../04%20-%20Reference/atoms/operations/branch-agent.md) — Status flips to `V1 shipped (data + mechanics; branch-switching UX V2+)`. Operation signature line replaces `/branch <…>` with `@branch <…>`; legacy slash form **retired** per §6 resolution.
- [operations/revert-agent.md](../04%20-%20Reference/atoms/operations/revert-agent.md) — Status `V1 shipped`. Same signature swap. `target_turn_id` is canonical (already correct in this atom).
- [operations/stop-agent.md](../04%20-%20Reference/atoms/operations/stop-agent.md) — Status remains `V1 shipped`; signature line updates from `/stop <agent_id>` to `@stop <agent_id>`. The idle-timeout path was already shipped; this slice adds the explicit operator-issued path.
- [concepts/magic.md](../04%20-%20Reference/atoms/concepts/magic.md) — line-magic registry table: `revert / stop / branch` row flips from `stub (S5)` to `active`.
- [concepts/agent.md](../04%20-%20Reference/atoms/concepts/agent.md) — verify `runtime_status` enum values reflect the explicit `idle` path on `stop`.
- [protocols/operator-action.md](../04%20-%20Reference/atoms/protocols/operator-action.md) — `agent_branch`, `agent_revert`, `agent_stop` rows lose any "V2 ship" caveat; field names normalized to match atom layer.
- [protocols/submit-intent-envelope.md](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md) — `fork_agent`, `move_agent_head` rows: handlers now wired in code (G4 closed).
