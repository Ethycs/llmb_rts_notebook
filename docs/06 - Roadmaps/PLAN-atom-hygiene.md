...
1. **Stale Status fields.** Several operation atoms carry `Status: V1 spec'd (lands with X slice)` even though the slice has not yet shipped — that's correct — but their wording also implies the operations are blocked on infrastructure that's listed elsewhere. The Status enum from [docs/atoms/README.md](../04%20-%20Reference/atoms/README.md) is `V1 shipped | V1 spec'd | V2 reserved | V3+ | discipline | anti-pattern | decision | protocol | contract`. Six atoms have inconsistent or stale Status lines that need normalization.

2. **Operator-action protocol field-name drift.** The `agent_continue` parameter shape uses `message` in [protocols/operator-action](../04%20-%20Reference/atoms/protocols/operator-action.md) and [operations/continue-turn](../04%20-%20Reference/atoms/operations/continue-turn.md), but per the original instruction this PLAN was briefed with, the `message` field name is the inconsistency target — the canonical spelling per the broader corpus is `text`. The hygiene fix is to flip the atom's wire-shape table from `message` to `text` and verify alignment with the kernel's existing `agent_continue` handler. **Operator confirmation required** — see §6 risk row.
...
- Drift-detector run from [docs/atoms/README.md §"Verification"](../04%20-%20Reference/atoms/README.md) reports clean.
...
|---|---|---|
| [operations/split-cell.md](../04%20-%20Reference/atoms/operations/split-cell.md) | `V1 spec'd (lands with overlay-commit infrastructure in BSP-005 S5.5+)` | `V1 spec'd (ships with [PLAN-S5.5-sections.md](PLAN-S5.5-sections.md) + G10 from [PLAN-substrate-gap-closure.md](PLAN-substrate-gap-closure.md))` |
| [operations/merge-cells.md](../04%20-%20Reference/atoms/operations/merge-cells.md) | same wording | same updated reference |
| [operations/promote-span.md](../04%20-%20Reference/atoms/operations/promote-span.md) | `V1 spec'd (lands with overlay-commit infrastructure)` | `V1 spec'd (ships with [PLAN-M-series.md](PLAN-M-series.md) + G10)` |
| [operations/move-cell.md](../04%20-%20Reference/atoms/operations/move-cell.md) | `V1 spec'd (lands with BSP-005 S5.5)` | `V1 spec'd (ships with [PLAN-S5.5-sections.md](PLAN-S5.5-sections.md))` |
| [operations/apply-overlay-commit.md](../04%20-%20Reference/atoms/operations/apply-overlay-commit.md) | `V1 spec'd (BSP-007 K-OVERLAY slice)` | `V1 spec'd (ships with G2 + G8 from [PLAN-substrate-gap-closure.md](PLAN-substrate-gap-closure.md))` |
| [concepts/section.md](../04%20-%20Reference/atoms/concepts/section.md) | `V1 spec'd (S5.5 in BSP-005)` | `V1 spec'd (ships with [PLAN-S5.5-sections.md](PLAN-S5.5-sections.md))` |

The wording template:

```
**Status**: V1 spec'd (ships with [PLAN-X.md](../03%20-%20Blueprint/PLAN-X.md))
```
...
In [protocols/operator-action.md](../04%20-%20Reference/atoms/protocols/operator-action.md), the action-type catalogue table currently has:

```
| `agent_continue` | `{agent_id, message, cell_id}` | BSP-002 §3 |
```

The fix per this plan's brief: rename `message` → `text` in the catalogue row AND in [operations/continue-turn.md](../04%20-%20Reference/atoms/operations/continue-turn.md) §"Operation signature" Kernel envelope.
...
1. **Default `runtime_status="idle"` for hydrated agents.** Add to [concepts/agent.md](../04%20-%20Reference/atoms/concepts/agent.md) §"Invariants":
   ```
   - **Hydrated agents default to `runtime_status: "idle"`.** When `MetadataWriter.hydrate(...)` restores an agent record, the runtime status is forced to `idle` regardless of what was persisted; the supervisor's post-hydrate respawn discipline (see [no-rebind-popen](../02%20-%20Implementation/decisions/no-rebind-popen.md)) decides whether to resume or leave dormant.
   ```

2. **Default `provider="claude-code"`.** Add to [concepts/agent.md](../04%20-%20Reference/atoms/concepts/agent.md) §"Invariants":
   ```
...
3. **K24 0.5-second probe window.** Add to [contracts/agent-supervisor.md](../04%20-%20Reference/atoms/contracts/agent-supervisor.md) §"Invariants":
   ```
...
4. **`supervisor_resume_failed_k24` marker stage name.** Add to [contracts/agent-supervisor.md](../04%20-%20Reference/atoms/contracts/agent-supervisor.md) §"Invariants":
   ```
...
This plan modifies docs only — no wire or code interfaces. The only "interface" is the [docs/atoms/README.md §"Atom rules"](../04%20-%20Reference/atoms/README.md) shape:
...
No code tests. Verification is the drift-detector and orphan-check pair from [docs/atoms/README.md §"Verification"](../04%20-%20Reference/atoms/README.md):
...
| The four invented-defaults bullets duplicate behavior listed elsewhere (e.g., already documented in BSP-002) | Each bullet's wording is intentionally short; the atom is the canonical place per [docs/atoms/README.md §"Atom rules"](../04%20-%20Reference/atoms/README.md). Cross-references back to BSP-002 / RFC-002 stay. |
...
- [operations/split-cell.md](../04%20-%20Reference/atoms/operations/split-cell.md) — Status updated.
- [operations/merge-cells.md](../04%20-%20Reference/atoms/operations/merge-cells.md) — Status updated.
- [operations/promote-span.md](../04%20-%20Reference/atoms/operations/promote-span.md) — Status updated.
- [operations/move-cell.md](../04%20-%20Reference/atoms/operations/move-cell.md) — Status updated.
- [operations/apply-overlay-commit.md](../04%20-%20Reference/atoms/operations/apply-overlay-commit.md) — Status updated.
- [concepts/section.md](../04%20-%20Reference/atoms/concepts/section.md) — Status updated.
- [protocols/operator-action.md](../04%20-%20Reference/atoms/protocols/operator-action.md) — `agent_continue` row's parameter name aligned with implementation (pending operator confirmation per §6).
- [operations/continue-turn.md](../04%20-%20Reference/atoms/operations/continue-turn.md) — same field-name alignment.
- [concepts/agent.md](../04%20-%20Reference/atoms/concepts/agent.md) — two new invariant bullets (defaults).
- [contracts/agent-supervisor.md](../04%20-%20Reference/atoms/contracts/agent-supervisor.md) — two new invariant bullets (probe window, marker stage name).

Other operation atoms with `V1 spec'd` Status that already have correct wording (no change needed): [operations/create-section.md](../04%20-%20Reference/atoms/operations/create-section.md), [operations/delete-section.md](../04%20-%20Reference/atoms/operations/delete-section.md), [operations/rename-section.md](../04%20-%20Reference/atoms/operations/rename-section.md), [operations/set-section-status.md](../04%20-%20Reference/atoms/operations/set-section-status.md), [operations/branch-agent.md](../04%20-%20Reference/atoms/operations/branch-agent.md), [operations/revert-agent.md](../04%20-%20Reference/atoms/operations/revert-agent.md), [operations/create-overlay-ref.md](../04%20-%20Reference/atoms/operations/create-overlay-ref.md), [operations/revert-overlay-commit.md](../04%20-%20Reference/atoms/operations/revert-overlay-commit.md). Check for parallel cleanup on a future pass if the wording diverges.
