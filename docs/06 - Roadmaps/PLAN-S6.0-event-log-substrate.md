...
|---|---|---|---|
| **Family A: `operator.action`** (Family D outer per [protocols/family-d-event-log](../04%20-%20Reference/atoms/protocols/family-d-event-log.md)) | inbound; every operator-typed event | yes | `wire/families.py:31`, `run_envelope.py:42` |
...
- `WIRE_VERSION = "1.0.0"` (`vendor/LLMKernel/llm_kernel/wire/version.py:3`). Major mismatch on a JSONL-load → reject with a structured error (matches the runtime [wire-handshake](../04%20-%20Reference/atoms/protocols/wire-handshake.md) major-mismatch behavior).
...
- [docs/atoms/protocols/family-f-notebook-metadata.md](../04%20-%20Reference/atoms/protocols/family-f-notebook-metadata.md) — Family F snapshot semantics (the load-checkpoint).
- [docs/atoms/protocols/family-d-event-log.md](../04%20-%20Reference/atoms/protocols/family-d-event-log.md) — Family D operator-action shape (every envelope this plan persists).
- [docs/atoms/protocols/wire-handshake.md](../04%20-%20Reference/atoms/protocols/wire-handshake.md) — version mismatch behavior the replayer mirrors on load.
- [docs/atoms/protocols/operator-action.md](../04%20-%20Reference/atoms/protocols/operator-action.md) — action_type catalogue persisted in the JSONL.
- [docs/atoms/protocols/submit-intent-envelope.md](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md) — intent envelope shape inside `zone_mutate` actions.
- [RFC-005 §"Persistence strategy"](../05%20-%20Standards/rfcs/RFC-005-llmnb-file-format.md) — JSON tree persistence (the JSONL is its peer).
- [RFC-006](../05%20-%20Standards/rfcs/RFC-006-kernel-extension-wire-format.md) — wire envelope contract.
