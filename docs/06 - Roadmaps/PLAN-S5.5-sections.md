...
**Goal**: implement `metadata.rts.zone.sections[]` per [section atom](../04%20-%20Reference/atoms/concepts/section.md) and [BSP-002 §13.1](../03%20-%20Blueprint/BSP-002-conversation-graph.md), including the four V1 status values as the interruptibility lock per [decisions/v1-section-status-interruptibility](../04%20-%20Reference/atoms/decisions/v1-section-status-interruptibility.md). Wire the create / rename / delete operations and the cell ↔ section dual-representation invariant.
...
- `metadata.rts.zone.sections[]` is real persisted state per [section atom §"Schema"](../04%20-%20Reference/atoms/concepts/section.md).
- Four operations land: `create_section`, `rename_section`, `delete_section`, `set_section_status` — all routed through `OverlayApplier`.
- `parent_section_id` slot exists but MUST be `null`; non-null is rejected per [decisions/v1-flat-sections](../04%20-%20Reference/atoms/decisions/v1-flat-sections.md).
...
- Status enum (`open | in_progress | complete | frozen`) is the V1 interruptibility lock per [decisions/v1-section-status-interruptibility](../04%20-%20Reference/atoms/decisions/v1-section-status-interruptibility.md). Every structural op on member cells gains a status precondition gated by K95.
- Extension renders sections as collapsible headers; clicking a header expands/collapses the contained cells.

### Non-goals

- Nested sections (V1.5+; the `parent_section_id` slot is reserved but inactive per [decisions/v1-flat-sections](../04%20-%20Reference/atoms/decisions/v1-flat-sections.md)).
...
1. **Persistence schema.** In `MetadataWriter`'s zone state, add `sections: list[dict]` per [section atom §"Schema"](../04%20-%20Reference/atoms/concepts/section.md). Hydrate path round-trips it; snapshot path emits it under Family F.

2. **Four overlay operations** — all sub-kinds inside `apply_overlay_commit` per [overlay-commit atom](../04%20-%20Reference/atoms/concepts/overlay-commit.md):
...
3. **Status machinery** per [decisions/v1-section-status-interruptibility](../04%20-%20Reference/atoms/decisions/v1-section-status-interruptibility.md) and [operations/set-section-status](../04%20-%20Reference/atoms/operations/set-section-status.md):
...
6. **Operator commands.** New VS Code commands wire to overlay-commit envelopes:
   ...
   - All produce `apply_overlay_commit` intents per [protocols/submit-intent-envelope](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md).
...
K-class additions (see [overlay-commit](../04%20-%20Reference/atoms/concepts/overlay-commit.md) and [decisions/v1-section-status-interruptibility](../04%20-%20Reference/atoms/decisions/v1-section-status-interruptibility.md)):
...
| Operator interrupts a frozen section by editing cell text directly | Cell text edits route through `cell_edit` action_type per [protocols/operator-action](../04%20-%20Reference/atoms/protocols/operator-action.md); the kernel rejects with K95 if section is frozen. |
...
- [concepts/section.md](../04%20-%20Reference/atoms/concepts/section.md) — Status flips from `V1 spec'd (S5.5 in BSP-005)` to `V1 shipped`.
- [decisions/v1-section-status-interruptibility.md](../04%20-%20Reference/atoms/decisions/v1-section-status-interruptibility.md) — referenced; no shape change. Verify the K95 sub-reasons match the implementation.
- [decisions/v1-flat-sections.md](../04%20-%20Reference/atoms/decisions/v1-flat-sections.md) — verify the validator behavior matches; no shape change.
- [operations/create-section.md](../04%20-%20Reference/atoms/operations/create-section.md), [operations/rename-section.md](../04%20-%20Reference/atoms/operations/rename-section.md), [operations/delete-section.md](../04%20-%20Reference/atoms/operations/delete-section.md), [operations/set-section-status.md](../04%20-%20Reference/atoms/operations/set-section-status.md) — all flip to `V1 shipped`.
- [contracts/overlay-applier.md](../04%20-%20Reference/atoms/contracts/overlay-applier.md) — Status flips from `V1 spec'd ... NOT yet present` to `V1 shipped` (assuming gap G8 closes here or before).
- [contracts/metadata-writer.md](../04%20-%20Reference/atoms/contracts/metadata-writer.md) — verify dual-representation invariant added to its invariants list.
- [concepts/cell.md](../04%20-%20Reference/atoms/concepts/cell.md) — `section_id` field now actively used.
