...
Driver: [KB-notebook-target.md §0.6](../03%20-%20Blueprint/KB-notebook-target.md), ratified into the slice ladder via [BSP-005 §6.2](../03%20-%20Blueprint/BSP-005-cell-roadmap.md#62-s35--contextpacker-simple-walker-new).
...
- A pure-function `ContextPacker.pack(notebook_state, current_cell, current_section) → ContextManifest` exists per [contracts/context-packer](../04%20-%20Reference/atoms/contracts/context-packer.md).
- The manifest is persisted via the new `record_context_manifest` intent kind per [protocols/submit-intent-envelope](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md).
...
- NO ranking, NO budget overflow, NO summary trust, NO retrieval. V1 is a structural walk only per [decisions/v1-contextpacker-walk](../04%20-%20Reference/atoms/decisions/v1-contextpacker-walk.md).
...
1. **New module.** Create `vendor/LLMKernel/llm_kernel/context_packer.py` per [contracts/context-packer](../04%20-%20Reference/atoms/contracts/context-packer.md). Implements the BSP-008 §3 algorithm verbatim:
   ```
...
3. **Output schema.** Match [concepts/context-manifest](../04%20-%20Reference/atoms/concepts/context-manifest.md) exactly. `manifest_id` is a fresh ULID; `total_token_estimate: None` per V1 lock.
...
The `ContextManifest` shape is locked by [concepts/context-manifest](../04%20-%20Reference/atoms/concepts/context-manifest.md) and MUST NOT diverge.
...
| Manifest size growth per run (large notebooks) | `total_turn_count` is bounded by the notebook size; V1 emits everything that passes filters per [decisions/v1-contextpacker-walk](../04%20-%20Reference/atoms/decisions/v1-contextpacker-walk.md). Budget overflow is V2 work. |
...
- [contracts/context-packer.md](../04%20-%20Reference/atoms/contracts/context-packer.md) — Status changes from `V1 spec'd ... NOT yet present` to `V1 shipped`. Update Status line and Code drift vs spec section.
- [concepts/context-manifest.md](../04%20-%20Reference/atoms/concepts/context-manifest.md) — Status `V1 shipped` already. Verify wording stays accurate.
- [decisions/v1-contextpacker-walk.md](../04%20-%20Reference/atoms/decisions/v1-contextpacker-walk.md) — no change; cite from this plan.
- [contracts/metadata-writer.md](../04%20-%20Reference/atoms/contracts/metadata-writer.md) — Code drift section: clear the line about `record_context_manifest` missing from `_BSP003_INTENT_KINDS`.
- [contracts/intent-dispatcher.md](../04%20-%20Reference/atoms/contracts/intent-dispatcher.md) — Code drift section: same.
- [protocols/submit-intent-envelope.md](../04%20-%20Reference/atoms/protocols/submit-intent-envelope.md) — registry table's `record_context_manifest` row's "atom" link now wires to a working implementation.
...
- [ ] [contracts/context-packer.md](../04%20-%20Reference/atoms/contracts/context-packer.md) Status flipped to `V1 shipped`.
