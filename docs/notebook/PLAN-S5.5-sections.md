...
**Status**: Phase 1a + 1b shipped (2026-05-16) — kernel CRUD ops via BSP-007 overlay applier (submodule `3a430cb`) + status state-machine via Phase 1b commit. `_validate_set_section_status` enforces the 4-status enum (`open | in_progress | complete | frozen`) and 8 allowed transitions; K95 `forbidden_section_transition` raised on disallowed transitions. 22 kernel tests (11 CRUD + 11 state-machine) in `test_overlay_applier.py`. **Phase 1c queued**: AgentSupervisor RunFrame auto-flip (open ↔ in_progress on run start/end, reference-counted), dual-representation invariant assertion at write time, extension header renderer + collapse + 4 commands. K-class shipped as **K90 / K95** (not K42 sub-reasons as the original design read) per the BSP-007 overlay-rejection scheme.
