...
**Status**: Phase 1a + 1b + 1c-kernel + 1d-auto-flip shipped (2026-05-16). Kernel-side of S5.5 is feature-complete. Phase 1a (CRUD) / 1b (status state-machine) / 1c (dual-rep invariant in `apply_commit`) / 1d-auto-flip (AgentSupervisor `_maybe_flip_section_on_run_start` / `_run_end` with reference counting via `_in_progress_run_count[section_id]`; submits `set_section_status` intents at 0↔1 ref-count boundaries; K95 from frozen sections logged via `_diagnostics.mark` but never blocks the run). 32 kernel tests total (11 CRUD + 11 state-machine + 4 dual-rep + 6 auto-flip). **Phase 2 queued (extension only)**: VS Code header renderer + collapse + 4 operator commands (`llmnb.section.create` / `.rename` / `.delete` / `.setStatus`). K-class shipped as **K90 / K95** (not K42 sub-reasons as the original design read) per the BSP-007 overlay-rejection scheme.
