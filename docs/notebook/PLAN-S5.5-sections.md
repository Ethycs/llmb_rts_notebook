...
**Status**: Phase 1a + 1b + 1c-kernel shipped (2026-05-16). Phase 1a (CRUD) + Phase 1b (status state-machine) via earlier commits. Phase 1c-kernel: dual-representation invariant enforced atomically inside `apply_commit` via `_assert_dual_representation_consistent` — after every op in a commit, `cells[<id>].section_id` and `sections[<id>].cell_range[]` MUST agree; mismatch fires K90 `section_membership_mismatch` with a `direction` detail field disambiguating which side observed the drift. 26 kernel tests total (11 CRUD + 11 state-machine + 4 dual-rep). **Phase 1d queued**: AgentSupervisor RunFrame auto-flip (open ↔ in_progress on run start/end, reference-counted via `_in_progress_run_count[section_id]`), extension header renderer + collapse + 4 commands. K-class shipped as **K90 / K95** (not K42 sub-reasons as the original design read) per the BSP-007 overlay-rejection scheme.
