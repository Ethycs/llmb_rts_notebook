...
**Status**: V1 shipped (2026-05-16) — kernel + 4 operator commands. **Phase 1** (kernel, feature-complete): 1a CRUD + 1b state-machine + 1c dual-rep invariant in `apply_commit` + 1d AgentSupervisor auto-flip via `_maybe_flip_section_on_run_start` / `_run_end` with reference counting; 32 kernel tests. **Phase 2** (extension commands shipped): `llmnb.section.create` / `.rename` / `.delete` / `.setStatus` Command Palette commands; envelope builders + DI prompt sinks + ~19 contract tests in `section-commands.test.ts`; commands ship `apply_overlay_commit` envelopes that the kernel-side validators consume directly. **Phase 3 queued (visual polish)**: VS Code section-header decoration (title above first cell + status badge color) + collapse handler — operator workflow works today via the Command Palette; the decoration is the affordance gap that would let operators discover sections without typing `llmnb.section.*` into the palette. K-class surface: **K90** (6 sub-reasons) + **K95** `forbidden_section_transition`.
