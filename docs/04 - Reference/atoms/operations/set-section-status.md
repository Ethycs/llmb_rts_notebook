...
**Status**: V1 shipped — operator-driven transitions via PLAN-S5.5 Phase 1b (`overlay_applier._validate_set_section_status` enforces the 4-value status enum and the allowed-transitions set per [decisions/v1-section-status-interruptibility](../decisions/v1-section-status-interruptibility.md); K90 on unknown section / invalid status value; K95 `forbidden_section_transition` on disallowed transitions); AgentSupervisor auto-flip via PLAN-S5.5 Phase 1d (`_maybe_flip_section_on_run_start` / `_run_end` increment-decrement a reference-counted `_in_progress_run_count[section_id]` and submit `set_section_status` intents at the 0↔1 boundaries — only the first run flips `open → in_progress`, only the last run flips back to `open`; K95 from a frozen section is logged via `_diagnostics.mark` but never blocks the run). 11 state-machine tests + 6 auto-flip tests.
**Source specs**: [BSP-007 §3.3](../../../03%20-%20Blueprint/BSP-007-overlay-git-semantics.md#33-section-level-new-per-kb-target-01-kb-target-6) (section-level operations), [decisions/v1-section-status-interruptibility](../decisions/v1-section-status-interruptibility.md) (the rule this enforces)
