...
- **Inspectability.** [Inspect mode](../../../03%20-%20Blueprint/KB-notebook-target.md#18-progressive-disclosure-modes) renders `text` directly with magic syntax highlighting; the operator never has to cross-reference a separate `kind` field to know what kind a cell is.
