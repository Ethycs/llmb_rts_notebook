...
**Source specs**: [PLAN-S5.0.1 §3.3](../../../07%20-%20Status%20Reports/PLAN-S5.0.1-cell-magic-injection-defense.md#33-output-sanitizer-wrapper--vendorllmkernelllm_kernelsocket_writerpy-modest) (the emission-ban chokepoint this standard inverts), [PLAN-S5.0.1 §3.7](../../../07%20-%20Status%20Reports/PLAN-S5.0.1-cell-magic-injection-defense.md#37-cell-manager-re-stamping) (`restamp_with_pin` as a certified emitter), [PLAN-S5.0 §3.8](../../../07%20-%20Status%20Reports/PLAN-S5.0-cell-magic-vocabulary.md#38-cell-manager-text-operations--vendorllmkernelllm_kernelcell_managerpy-new-80-loc) (Cell Manager text primitives), [PLAN-S5.0.2](../../../07%20-%20Status%20Reports/PLAN-S5.0.2-magic-code-generators.md) (generator handlers, queued)
...
| Magic code generator handlers (V1 shipped, S5.0.2) | `@@template` / `@@expand` / `@@import` per [PLAN-S5.0.2](../../../07%20-%20Status%20Reports/PLAN-S5.0.2-magic-code-generators.md) | new cells inserted after the generator cell | `emit_magic_line()`; mandatory `generated_by` provenance |
| Privileged agent via MCP tool (V1 shipped, S5.0.4) | agent invokes `emit_magic_cell(name, args, …)` per [protocols/mcp-tool-call](../protocols/mcp-tool-call.md); kernel handler routes through Cell Manager | a new cell with `@@<name> <args>` and `generated_by: <agent_id>` provenance; `promoted_from_stream: true` on the stream-promotion path | `emit_magic_line()` inside the kernel-side tool handler; the agent never sees the hash. K3K rejects unprivileged invocations; K3L marks stream-promotion-eligible emissions |
...
The kernel's emission ban remains non-negotiable on the prose surface, even for privileged agents; the operator's only escape hatch from the ban itself is the verbatim acceptance string per [PLAN-S5.0.1 §3.11](../../../07%20-%20Status%20Reports/PLAN-S5.0.1-cell-magic-injection-defense.md#311-refusal-flag--verbatim-injection-acceptance), which doesn't certify anything — it merely records that the operator has opted out of protection.

## V1 vs V2+

- **V1 (post-S5.0.1)**: operator typing, Cell Manager primitives, `restamp_with_pin`, writer serializer.
- **V1 (post-S5.0.2)**: generator handlers ship per [PLAN-S5.0.2](../../../07%20-%20Status%20Reports/PLAN-S5.0.2-magic-code-generators.md).
- **V1 (post-S5.0.4) — shipped**: privileged-agent emission lands as the `emit_magic_cell` MCP tool — operator grants the privilege via an operator-action intent (`grant_magic_emit_privilege { agent_id, zone_id, scope }`), the kernel records it in `metadata.rts.config.magic_emit_privileges[]`, and the supervisor exposes the tool only to the named agent. Privileged emissions appear in History mode like any overlay commit and carry `generated_by: <agent_id>` provenance. K3K rejects unprivileged invocations; K3L marks the stream-promotion case so the audit trail distinguishes invoked-tool from promoted-stream cells. The promotion chip is the operator-action `promote_stream_magic { cell_id, line }` that synthesizes the tool call.
- **V2+**: per-tool capability scopes beyond the V1 `magics: [name, ...] | "all"` shape; agent-as-generator pattern (a generator handler that runs the privileged tool on the operator's behalf, composing the two carve-outs).
- **V3+**: per-emitter audit log surfacing every line that flowed through a certified path; cross-notebook generator pipelines.
...
- [PLAN-S5.0.1](../../../07%20-%20Status%20Reports/PLAN-S5.0.1-cell-magic-injection-defense.md) — defines `restamp_with_pin`, `emit_magic_line()`, the emission-ban sanitizer, and K3A/K3I/K3J
- [PLAN-S5.0.2](../../../07%20-%20Status%20Reports/PLAN-S5.0.2-magic-code-generators.md) — queued slice for generator handlers
