...
The span inherits cell + agent attribution from the parent turn via `llmnb.agent_id` and `llmnb.cell_id` ([RFC-006 §1](../../../05%20-%20Standards/rfcs/RFC-006-kernel-extension-wire-format.md)).
...
- **The `tool` cell-kind is reserved.** V1 directive parser ([BSP-002 §3](../../../03%20-%20Blueprint/BSP-002-conversation-graph.md#3-cell-directive-grammar)) does NOT recognize a `tool` directive. V1 receivers seeing a cell with `kind: "tool"` render it inert and dispatch nothing.
...
- **V2+**: the `tool` cell-kind activates with a control directive (e.g., `/run tests`). The tool's invocation, arguments, and result become the cell's primary content; no parent agent turn is required. The device-call mechanism ([KB-target §11](../../../03%20-%20Blueprint/KB-notebook-target.md#11-tools-as-devices)) wires through the kernel.
