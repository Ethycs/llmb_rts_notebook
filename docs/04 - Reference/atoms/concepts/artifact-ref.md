...
- **V2+**: `body: null` permitted. Adds `byte_index`, `line_index`, `semantic_index`, `loaded_windows[]`, `pinned_ranges[]`, `summaries[]` per [KB-target §16](../../../03%20-%20Blueprint/KB-notebook-target.md#16-artifact-streaming). The `artifact` cell-kind activates with the streaming materializer; cells become portals (viewport → range resolver → window fetch → renderer).
