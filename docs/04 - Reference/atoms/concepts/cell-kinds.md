...
| `markdown` | **Shipped** | Operator prose / notes; no agent, no execution. (= comment cell M1 from [BSP-005](../../../03%20-%20Blueprint/BSP-005-cell-roadmap.md).) | `bound_agent_id` MUST be absent or null. |
...
| `checkpoint` | **Shipped** | Summarizes a contiguous range or section. Substitutes its summary for underlying turns when included in agent context. | SHOULD carry `summary_text` + `covers_cell_ids[]`. Operator-authored only in V1; AI-summarized → V2 ([KB-target §22.6](../../../03%20-%20Blueprint/KB-notebook-target.md#226-checkpoint-trust-model)). |
...
| `artifact` | **Reserved** | Displays / lenses a single [ArtifactRef](artifact-ref.md) range. | V2+ — depends on streaming materializer ([KB-target §0.9](../../../03%20-%20Blueprint/KB-notebook-target.md#09-artifacts--v1-ships-the-shape-v2-ships-streaming)). |
...
- **Unknown values are rejected.** Anything outside the eight values above triggers a `wire-failure` LogRecord ([RFC-006 §"Failure modes" W4](../../../05%20-%20Standards/rfcs/RFC-006-kernel-extension-wire-format.md)).
