...
**`AgentSupervisor.respawn_from_config` re-spawns claude-code processes from `claude_session_id` via `--resume`. It NEVER rebinds to a live PID.** Per [BSP-002 §4](../../../03%20-%20Blueprint/BSP-002-conversation-graph.md#43-idle-exit) and the agent schema in [BSP-002 §2.2](../../../03%20-%20Blueprint/BSP-002-conversation-graph.md#22-agent):
