...
1. **The merge-correctness rule [KB-target §22.1](../../../03%20-%20Blueprint/KB-notebook-target.md#221-splitmerge-invariants) requires "same zone or compatible parent zone."** With a single level, "compatible parent" reduces to "same section" — no ambiguity. Nesting introduces "merge cell from `Architecture > Runtime` with cell from `Architecture > Storage`?" — a question we don't want to answer in V1.
