...
The `app.py` / `boot_kernel` / `shutdown_kernel` scaffolding from BSP-004 stays in tree because it is reusable by the eventually-embedded ASGI runtime ([asgi-deferred](asgi-deferred.md), [BSP-006](../../../03%20-%20Blueprint/BSP-006-embedded-asgi.md)).

## Rationale

1. **Tier 4 green is the V1 ship gate.** Per [BSP-004 V1.x retrospective](../../../03%20-%20Blueprint/BSP-004-kernel-runtime.md): the V1 attempt ran 1.4s before `async_serve_socket_exited`, after which the operator's `notify` span never reached the cell — Tier 4 timed out at 180s where the prior session ran in 8.7s. The V2 attempt regressed differently (proxy startup `FileNotFoundError` inside lifespan). Legacy `main()` ran 4/4 green on 2026-04-28.

2. **The bugs are Windows subprocess + handle interactions, not "uvicorn vs hypercorn."** Per [BSP-006 §2](../../../03%20-%20Blueprint/BSP-006-embedded-asgi.md): switching ASGI servers doesn't fix the underlying `subprocess.Popen` issues on Windows. The cure was twofold:
...
| No `/health` endpoint, no FastAPI, no uvicorn import on kernel boot | [BSP-006 §7](../../../03%20-%20Blueprint/BSP-006-embedded-asgi.md) |
...
- **V3+**: fleet kernels (one per zone) trigger the embedded ASGI per [BSP-006](../../../03%20-%20Blueprint/BSP-006-embedded-asgi.md). Legacy `main()` MAY remain for non-fleet dev paths.
