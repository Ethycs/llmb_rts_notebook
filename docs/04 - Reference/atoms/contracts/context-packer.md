...
- **Deterministic.** Same `(notebook_state, current_cell, current_section)` → byte-identical manifest. This is the precondition for replay correctness and for [Inspect mode](../../../03%20-%20Blueprint/KB-notebook-target.md#18-progressive-disclosure-modes) "explain why."
...
- **Validation is delegated.** The packer does not validate `cell_refs` against persisted state — that happens at write time in `MetadataWriter` per [BSP-003 §6 step 4](../../../03%20-%20Blueprint/BSP-003-writer-registry.md#6-apply-discipline) (K103 on unknown refs).
