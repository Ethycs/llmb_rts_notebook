...
**Source specs**: [RFC-006 §5](../../../05%20-%20Standards/rfcs/RFC-006-kernel-extension-wire-format.md#5--family-c-agent-graph), [RFC-005 §`metadata.rts.agents`](../../../05%20-%20Standards/rfcs/RFC-005-llmnb-file-format.md#metadatartsagents--agent-state-graph)
...
Node and edge schemas are exactly those in [RFC-005 §`metadata.rts.agents`](../../../05%20-%20Standards/rfcs/RFC-005-llmnb-file-format.md#metadatartsagents--agent-state-graph). The response's `correlation_id` MUST equal the originating query's; the extension's pending-query table is keyed on it.
