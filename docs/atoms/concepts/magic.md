...
| `section` | active | `@@section <title> [id:"sec_..."]` — declares an empty section via `apply_overlay_commit` with a `create_section` op. Title from positional[0] or `title:` named arg; explicit `id:` pins `section_id` (otherwise minted from title). Population (cells joining the section) is a separate gesture: Command Palette `llmnb.section.openActions` or the `move_cells_into_section` overlay op. CLI driver (`llmnb execute`) ships envelopes via `executor._derive_cell_envelope` (PLAN-S5.5 Phase 4); kernel-side parser typed args via `magic_registry._SectionCellMagic`. **Rendered as markdown** (PLAN-S5.5 Phase 5): `magic_to_llmnb` emits `cell_type: "markdown"` with synthesized `source: "# <title>"` so VS Code's native markdown-header folding handles section-range collapse without a custom API. The canonical `@@section <title>` magic text is preserved verbatim in `metadata.rts.cells[<id>].text` for round-trip identity. |
