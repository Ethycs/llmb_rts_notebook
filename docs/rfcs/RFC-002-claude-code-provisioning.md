...
3. **Stream parsing.** The kernel reads `stdout` line by line. Each line MUST be one of: a Claude stream-json record, an MCP JSON-RPC frame routed through Claude's MCP plumbing, or empty. Any line that fails both parsers is a free-form-text emission and constitutes a **DR-0010 violation**: the kernel MUST log it as a synthetic `notify(observation="agent emitted prose: <line>", importance="medium")` and MUST NOT surface the raw text to the operator. If violations exceed five per minute, the kernel MUST escalate via a synthetic `escalate(reason="repeated DR-0010 violations", severity="medium")`.
4. **stderr capture.** All `stderr` is captured by the kernel and MUST NEVER reach the operator surface. The kernel writes stderr to its own log file, indexed by agent_id.
...
| Agent emits prose despite system prompt | Stdout line that parses as neither stream-json nor MCP JSON-RPC | Log as synthetic `notify`; do NOT surface to operator unless flood threshold (>5/min) hit; on flood, `escalate` | none below threshold; escalate above | `dr0010.violation`, `dr0010.flood` |
...
| Allowed-tools restriction bypass attempt | Agent emits a tool call for a tool not in `mcpServers.*.allowedTools` | Kernel MCP server MUST return JSON-RPC `-32601 method not found`; log; do NOT auto-terminate | none in V1 (logged only) | `dr0010.bypass_attempt` |
| Stderr containing prose | Common from `claude` itself; not a DR-0010 violation | Capture to kernel log; never surface | none | `agent.stderr` (debug-only) |
