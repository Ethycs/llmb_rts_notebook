...
[DR-0011](../docs/02%20-%20Implementation/decisions/0011-subtractive-fork-vscode-jupyter.md)
and [chapter 07](../docs/01%20-%20Design/dev-guide/07-subtractive-fork-and-storage.md).
...
- **S1** cell-as-agent identity badges (gutter color + status bar +
  kind label) — [docs/atoms/concepts/cell.md](../docs/04%20-%20Reference/atoms/concepts/cell.md).
...
- **S5.0.3.x** headless executor support: shares the cell-magic parser +
  ...
  ([discipline/certified-magic-emitter](../docs/04%20-%20Reference/atoms/discipline/certified-magic-emitter.md)). Talks to LLMKernel over
  TCP transport ([RFC-008 v1.0.1](../docs/05%20-%20Standards/rfcs/RFC-008-kernel-host-integration.md)) with the v2.1.0 wire-handshake envelope
  ([protocols/wire-handshake](../docs/04%20-%20Reference/atoms/protocols/wire-handshake.md)).
...
- **Inspect mode V1** (commit `92c7412`) — read-only per-cell view of
  ...
  [atoms/concepts/context-manifest.md](../docs/04%20-%20Reference/atoms/concepts/context-manifest.md)).
  Cell-toolbar status item shows `▶ <run_id> (<status>) · N cells / K
  excluded`; click opens a manifest detail QuickPick rendering the
  inclusion/exclusion trace per [BSP-008 §11](../docs/03%20-%20Blueprint/BSP-008-contextpacker-runframes.md).
...
See [docs/notebook/BSP-005-cell-roadmap.md §6.5](../docs/03%20-%20Blueprint/BSP-005-cell-roadmap.md#65-slice-ladder-totals-after-issue-2--and-observed-velocity-2026-05-02-update) for the
...
The extension talks to LLMKernel over the [RFC-008](../docs/05%20-%20Standards/rfcs/RFC-008-kernel-host-integration.md) PTY+socket transport
